    ...
    <div className='bg-white p-3 rounded-md border border-gray-300 flex justify-between'>

      {
        changeTitle ? <div className='flex md:items-center md:gap-2 flex-col md:flex-row w-1/2 md:w-1/4 gap-y-2 md:gap-y-0'>
          ...
          <MdOutlineDone className='text-green-800 text-md cursor-pointer' onClick={() => {setChangeTitle(!changeTitle);}} />
        </div> :
      ...
      <div className='flex gap-2'>
        <button className='resume-btn' onClick={() => open(true)}><span className='lg:block hidden'>Change Theme</span></button>
        <button className='resume-btn' onClick={handleResumeDelete}><span className='lg:block hidden'>Delete</span></button>
        <button className='resume-btn' onClick={() => {}}><span className='lg:block hidden'>Download</span></button>
      </div>
    </div>
  )
}
